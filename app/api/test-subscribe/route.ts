...
// Map plan IDs to database enum values
function mapPlanTypeToEnum(planId: string) {
  ...
  return mapping[planId] || 'TWELVE_MONTHS'; // Default fallback
}
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const subscription = await tx.subscription.create({
        data: {
          ...
          planType: mapPlanTypeToEnum(planType),
          status: 'ACTIVE',
